...
from .base import actionengine_tool, ToolResult
from .run import run_command, run_command_with_input
...
class TerminalSession:
    ...
    _instance: Optional[asyncio.subprocess.Process] = None
    ...
    @classmethod
    async def execute(
        cls, command: str, input_text: Optional[str] = None
    ) -> ToolResult:
        ...
        async with cls._lock:
            try:
                ...
                if command.strip().startswith("cd "):
                    return await cls._handle_cd(command)

                # Add current directory context
                result = await (
                    run_command_with_input(command, input_text, cwd=cls._current_dir)
                    if input_text
                    else run_command(command, cwd=cls._current_dir)
                )
                ...
                return ToolResult(
                    error=str(e), system=f"Failed to execute in {cls._current_dir}"
                )

    @classmethod
    async def _handle_cd(cls, command: str) -> ToolResult:
        ...
        try:
            ...
            if path == "~":
                ...
                if not os.path.isabs(path):
                    path = os.path.join(cls._current_dir, path)
            ...
            cls._current_dir = new_path
            return ToolResult(
                output=f"Changed directory to: {new_path}", system=f"CWD: {new_path}"
            )

        except Exception as e:
            return ToolResult(error=f"Failed to change directory: {str(e)}")


@tool
async def terminal_tool(command: str) -> ToolResult:
    ...
    try:
        ...
        current_terminal_id = await TerminalManager().get_current_terminal_id()
        mgr = TerminalManager()

        output, success = await mgr.execute_command(current_terminal_id, command)
